...
{
    private TemplateVariableMapper $mapper;

    public function __construct(TemplateVariableMapper $mapper)
    {
        $this->mapper = $mapper;
    }
    ...
    public function generateDocument(
        ...
        // Mapper TOUTES les données du client aux variables du template via le système de mapping
        $variables = $this->mapper->mapVariables($client);

        // Log des statistiques de mapping
        $stats = $this->mapper->getMappingStats();
        Log::info("Document generation mapping stats", $stats);
        ...
        $fileName = $this->generateFileName($client, $template, $format);
     ...
     * Note: Cette méthode nécessite LibreOffice ou une autre solution de conversion
     */
    private function convertToPdf(string $docxPath): string
    {
        // Pour l'instant, on garde le DOCX
        // La conversion PDF nécessite LibreOffice headless ou une bibliothèque externe
        // Exemple avec LibreOffice:
        // exec("libreoffice --headless --convert-to pdf --outdir " . dirname($docxPath) . " " . $docxPath);

        // Pour cette implémentation, on retourne le DOCX tel quel
