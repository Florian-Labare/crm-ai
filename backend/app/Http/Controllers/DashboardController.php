...
class DashboardController extends Controller
{
    ...
    private function normalizeBesoin(string $besoin): string
    {
        ...
        if (str_contains($lower, 'retraite') || preg_match('/\bper\b/', $lower)) return 'Retraite';
        if (str_contains($lower, 'prévoyance') || str_contains($lower, 'prevoyance') || str_contains($lower, 'décès')) return 'Prévoyance';
        if (str_contains($lower, 'santé') || str_contains($lower, 'sante') || str_contains($lower, 'mutuelle')) return 'Santé';
        if (str_contains($lower, 'emprunt') || str_contains($lower, 'crédit')) return 'Emprunteur';
        if (str_contains($lower, 'épargne') || str_contains($lower, 'epargne') || str_contains($lower, 'assurance vie') || str_contains($lower, 'placement')) return 'Épargne';
        return 'Autre';
    }
}
